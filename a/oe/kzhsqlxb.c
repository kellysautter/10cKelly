				  ...
				  vDTE, "TE_FieldDataRel", "ZKey", "TE_TablRec") >= 0)
			  {
				  if (CheckExistenceOfEntity(vDTE, "ER_Entity") >= zCURSOR_SET)
					  ...
					  if (CheckExistenceOfEntity(vDTE, "ER_EntIdentifier") >= zCURSOR_SET &&
						  ...
						  zstrcat(pchEnd, " int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE)");
					  else
						  zsprintf( pchEnd, " INTEGER" );
				  else
					  zsprintf( pchEnd, " INTEGER" );
   ...
   if ( fnWriteLine( vDTE, f, szLine ) < 0 )
      goto EndOfFunction;
       ...
       if ( pchSchemaName[0] == 0 )
          ...
          zsprintf(szLine, "SET SCHEMA '%s' %s", pchSchemaName, LINE_TERMINATOR);
       if ( fnWriteLine( vDTE, f, szLine ) < 0 )
         goto EndOfFunction;

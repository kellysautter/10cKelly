...
// zSHORT    (POPERATION pfn)( zVIEW, zVIEW, zVIEW, zPCHAR, zVIEW );
   ...
   SetNameForView( vEMD, "CurrentERD", vSubtask, zLEVEL_TASK );
   ...
   if ( hLibrary )
   {
      pfn = (zFARPROC_VVVPV) SysGetProc( hLibrary, "BuildSchema" );
      if ( pfn )
      {
         zVIEW vTaskLPLR;
         ...
         GetViewByName( &vDTE_Import, "TZRetrieveSchema", vSubtask, zLEVEL_TASK );
         oTZEREMDO_ImportDTE_DataToERD( vEMD, vDTE, vDTE_Import );
